...
CREATE TABLE webapp.search_history (
    ...
    CONSTRAINT search_history_status_check CHECK (((status)::text = ANY ((ARRAY['running'::character varying, 'completed'::character varying, 'failed'::character varying, 'cancelled'::character varying, 'finished'::character varying])::text[])))
);
