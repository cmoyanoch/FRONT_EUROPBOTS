...
CREATE TABLE IF NOT EXISTS webapp.leads (
    ...
    -- Campo para ID de contenedor de Phantombuster
...
CREATE INDEX IF NOT EXISTS idx_leads_containerId ON webapp.leads(containerId);
...
CREATE OR REPLACE FUNCTION webapp.insert_lead(
    ...
    p_containerId VARCHAR(20) DEFAULT NULL
)
RETURNS UUID AS $$
DECLARE
    lead_id UUID;
BEGIN
    ...
    INSERT INTO webapp.leads (
        ...
        user_id, id_crm, process, containerId
    ) VALUES (
        ...
        p_user_id, 1, 'initial', p_containerId
