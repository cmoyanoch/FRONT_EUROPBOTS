...
export default function ConfigPage() {
  ...
  const handleTestSearchConnection = async () => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        alert('Conexión exitosa! El webhook de búsqueda está funcionando correctamente.')
      } else {
        const errorMessage = result.message || 'Error al probar la conexión del webhook de búsqueda'
        alert(`Error: ${errorMessage}`)
      }
    } catch (error) {
      console.error('Error testing search webhook:', error)
      alert('Error de conexión. Verifica que la URL sea correcta y el servidor esté disponible.')
    } finally {
  ...
  const handleTestCampaignConnection = async () => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        alert('Conexión exitosa! El webhook de campaña está funcionando correctamente.')
      } else {
        const errorMessage = result.message || 'Error al probar la conexión del webhook de campaña'
        alert(`Error: ${errorMessage}`)
      }
    } catch (error) {
  ...
  useEffect(() => {
    const loadWebhookConfig = async () => {
      try {
        console.log('Cargando configuración de los webhooks...')
        ...
        if (searchResponse.ok) {
          ...
          if (searchData.success && searchData.webhookUrl) {
            console.log('Estableciendo URL del webhook de búsqueda:', searchData.webhookUrl)
            setSearchWebhookUrl(searchData.webhookUrl)
          }
        }
        
        // Cargar webhook de campaña
        const campaignResponse = await fetch('/api/config/webhook?type=campaign')
        if (campaignResponse.ok) {
          const campaignData = await campaignResponse.json()
          if (campaignData.success && campaignData.webhookUrl) {
            console.log('Estableciendo URL del webhook de campaña:', campaignData.webhookUrl)
  ...
  const renderApiTab = () => (
    <div className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center space-x-2">
                ...
                >
                  ...
                  <span>{isSavingSearchWebhook ? 'Guardando...' : 'Guardar Webhook'}</span>
                </button>
                <button 
            ...
            {searchWebhookStatus === 'error' && (
              <div className="mt-3 p-3 bg-red-500/10 border border-red-500/20 rounded-lg">
                <p className="text-sm text-red-300">
                  ❌ Error al guardar el webhook de búsqueda. Inténtalo de nuevo.
                </p>
              </div>
            )}
      ...
      <div className="bg-white/10 backdrop-blur-sm rounded-xl border border-europbots-secondary/20 p-6">
        <h3 className="text-lg font-semibold text-white mb-4">Webhook del Bot de Crear Campaña</h3>
        <div className="space-y-4">
          <div className="p-4 bg-white/5 rounded-lg">
            <div className="flex items-center justify-between mb-2">
              <p className="text-white font-medium">URL del Webhook</p>
              <button 
                ...
                className="text-europbots-secondary hover:text-europbots-secondary/80 text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isTestingCampaignConnection ? 'Probando...' : 'Probar Conexión'}
              </button>
            </div>
            <div className="space-y-3">
              <input
              ...
              <div className="flex items-center space-x-2">
                ...
                >
                  <Save className="w-4 h-4" />
                  <span>{isSavingCampaignWebhook ? 'Guardando...' : 'Guardar Webhook'}</span>
                </button>
                <button 
            ...
            <div className="mt-3 p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg">
              <p className="text-sm text-blue-300">
                <strong>Nota:</strong> Esta URL recibirá notificaciones automáticas cuando se cree una nueva campaña.
              </p>
            </div>
          </div>
        </div>
